...
// Uses calculated_price; shows compare only when it's a sale price
function extractPrices(p: HttpTypes.StoreProduct) {
  ...
  const toMajor = (n: number) => n; // keep as-is if backend already returns major units
...
export default function CategoryPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const cat = product_categories?.[0] ?? null;
      setCategory(cat);
      setProducts([]);
      setPage(1);
      setCount(0);
  ...
  // Fetch products for category (by id) when category or page changes
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="max-w-6xl mx-auto w-full px-4 md:px-8 py-6 flex-1">
        {!category && !loading && (
          <div className="text-sm text-text-mutable">Category not found.</div>
        )}
